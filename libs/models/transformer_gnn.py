...
from allennlp.training import metric_tracker, trainer

from libs.modules.gnn_encoders import GNNEncoder
from libs.tools.gnn_utils import create_torch_geometric_batch


logger = logging.getLogger(__name__)


@Model.register("ssqa_gnn")
class TransformerGNN(Model):
    """
        ...
        **kwargs
        ...
        self._text_field_embedder = BasicTextFieldEmbedder(
            {"tokens": PretrainedTransformerMismatchedEmbedder(
                transformer_model_name, max_length=512)}
        )
        ...
        if self.gnn_encoder:
            self._linear_layer = nn.Linear(
                self._text_field_embedder.get_output_dim()+self.gnn_encoder.get_output_dim(), 2)
        else:
            self._linear_layer = nn.Linear(
    ...
    def forward(  # type: ignore
        ...
        cross_edges: Optional[torch.LongTensor] = None,
        ...
        best_span_str : `List[str]`
            ...
            question.
        """
        bert_representations = self._text_field_embedder(question_with_context)

        if self.gnn_encoder:
            # GNN output
            graph_data = create_torch_geometric_batch(
                bert_representations, edges, cross_edges)
            gnn_representations = self.gnn_encoder(graph_data)
            gnn_representations = torch.stack(gnn_representations.split(
                gnn_representations.size(0)//bert_representations.size(0)
            ))

            logits = self._linear_layer(
                torch.cat([bert_representations, gnn_representations], -1))
        else:
            logits = self._linear_layer(bert_representations)
        ...
        possible_answer_mask = torch.zeros_like(
            span_end_logits, dtype=torch.bool
        )
        ...
        span_start_probs = torch.nn.functional.softmax(
            span_start_logits, dim=-1)
        ...
        if answer_span is not None:
            ...
            self._span_accuracy(
                best_spans, answer_span, span_mask.unsqueeze(
                    -1).expand_as(best_spans)
            )

            start_loss = cross_entropy(
                span_start_logits, span_start, ignore_index=-1)
            ...
            end_loss = cross_entropy(
                span_end_logits, span_end, ignore_index=-1)
            ...
            for metadata_entry, best_span, cspan in zip(metadata, best_spans, context_span):
                ...
                if predicted_end >= len(context_tokens_for_question):
                    ...
                    character_end = end_token.idx + \
                        len(sanitize_wordpiece(end_token.text))

                # best_span_string = metadata_entry["context"][character_start:character_end]
                best_span_string = "".join(x.text for x in
                                           context_tokens_for_question[predicted_start:predicted_end+1])
